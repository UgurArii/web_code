...
if(isset($_POST['markaEkleSubmit'])){
    /*form değer alınmasında
    print_r($_POST);
    echo"<br>";
    print_r($_FILES);
    */
    $marka = postValues($_POST['Marka']);
    ...
    $query_MarkaEkle = "INSERT INTO urun_marka(Marka,MarkaLogo,HarfID,Listeleme) VALUES ('$marka','$markaLogo','$harfID','$listeleme')";
    $result_MarkaEkle = mysql_query($query_MarkaEkle);
    
    if($result_MarkaEkle){
        //marka veritabanına başarıyla eklenmesi
        $filename = $_FILES['MarkaLogo']['tmp_name'];
        $destination = "../../_uploads/resim/urun-marka/.$markaLogo";
        $sonuc = move_uploaded_file($filename, $destination);
     ...
     header("Location:index.php?Islem=MarkaEkle");
}
?>
<!DOCTYPE html>
<html>
...
<section>
    <h1>Ürün > Marka Ekle</h1>
    <form action="<?= $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
        <fieldset>
            <legend>Harf Bilgileri</legend>
            <label for="HarfID">Harf Bilgileri</label>
            <select id="HarfID" name="HarfID">
                <?php do { ?>
                <option value="<?= $row_rsUrunMarkaHarf->HarfID;?>"><?= $row_rsUrunMarkaHarf->Harf?></option>
                
                <?php } while($row_rsUrunMarkaHarf = mysql_fetch_object($rsUrunMarkaHarf)); ?>
            </select>
            <label for="Listeleme">Listelensin mi?</label>
            <input type="checkbox" name="Listeleme" id="Listeleme"/>
        </fieldset>
        ...
        <fieldset>
            ...
            <br>
            <input type="submit" name="markaEkleSubmit" value="Marka Ekle"/>   
        </fieldset>
    </form>
    <br/>
</section>
